...
import { Menu, X, ChevronDown, Layers, Briefcase, MessageCircle } from "lucide-react";
...
export const NavbarByAnima = (): JSX.Element => {
  ...
  const navLinks = [
    ...
    {
      title: "Case Studies", href: "#", dropdown: [
        "By Industry",
        "By Business Size",
        "Client Stories"
      ]
  ...
  return (
    <div className="pt-[80px]">
      ...
      <motion.header
        ...
        className={`fixed top-[32px] left-0 right-0 z-40 w-full transition-all duration-300 font-jost bg-[#1d002c] text-white text-sm border-b border-[#404040]`}
      >
        <nav
          role="navigation"
                ...
                <div className="flex flex-col p-6 space-y-4">
                  {navLinks.map((link) => (
                    <Link
                      key={link.href}
                      to={link.href}
                      className={`text-white font-medium hover:text-purple-300 transition-colors text-sm ${location.pathname === link.href ? "text-app-primary" : ""}`}
                  ...
                  <div className="pt-4 border-t border-gray-200">
                    <Button className="w-full bg-app-primary hover:bg-app-secondary text-white text-lg py-3">
                      Sign Up
                    </Button>
                  </div>
                </div>
          ...
          <div className="hidden md:flex items-center space-x-4 lg:space-x-6" ref={dropdownRef}>
            {navLinks.map((link) => (
              <div key={link.title} className="relative">
                <button
                  onClick={() =>
                    setActiveDropdown((prev) => (prev === link.title ? null : link.title))
                  }
            ...
            <div className="flex items-center space-x-4">
              <Button variant="ghost" className="text-white hover:text-purple-300 text-sm">
                Custom Request
              </Button>
              <Button
